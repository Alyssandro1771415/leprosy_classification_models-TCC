...
# -*- coding: utf-8 -*-
"""
Gera figuras Grad-CAM em ml/artifacts/figures a partir dos modelos treinados.
"""
import argparse
import csv
import os
...
from leprosy_ml.paths import data_dir, figures_dir, get_ml_root, metrics_dir, models_dir
from leprosy_ml.preprocessing.pipeline import rgb_to_y_channel
from leprosy_ml.visualization.gradcam import (
...
def main():
    ...
    raw_val = data_dir("co2wounds_v2", "raw", "train_images_binary", "val")
    proc_val = data_dir("co2wounds_v2", "processed", "train_images_binary", "val")
    out_base = figures_dir()

    class_names = ["leprosy", "outros"]

    keras_paths = sorted(models_dir("co2wounds").glob("*.keras"))
    if not keras_paths:
        print("Nenhum arquivo .keras em artifacts/models/co2wounds/.")
        return

    manifest_path = metrics_dir() / "predictions_manifest.csv"
    skipped_log = metrics_dir() / "skipped_pairs.log"
